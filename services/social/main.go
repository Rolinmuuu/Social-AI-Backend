...
import (
	...
	"socialai/services/social/handler"
...
func main() {
	...
	addr := ":8083"
	logger.Logger.Info("social-service starting", zap.String("addr", addr))
	if err := http.ListenAndServe(addr, handler.InitRouter(esBackend)); err != nil {
		logger.Logger.Fatal("social-service stopped", zap.Error(err))
	}
}
